...
| 카테고리 | 명령 | 역할 | 산출물 |
|---|---|---|---|
| 기획 | `$deep-interview` | 요구사항 심층 질문 | `.omx/specs/` |
| 기획 | `$ralplan` | 합의 기반 계획 수립 (`$plan`의 별칭) | `.omx/plans/` |
| 기획 | `$plan` | 계획 정책과 승인 게이트 (정식 명칭) | `.omx/plans/` |
| 실행 | `$autopilot` | 완전 자율 루프: ralplan → ralph → code-review | 검증된 완료 상태 |
| 실행 | `$ralph` | 지속 실행 루프 (ultrawork 포함) | 검증된 완료 상태 |
| 실행 | `$team` | tmux 기반 다중 worker 실행 | `.omx/logs/`, `.omx/state/` |
| 실행 | `$ultragoal` | 다중 목표 장기 추적 | `.omx/ultragoal/` |
| 실행 | `$pipeline` | 구성 가능한 파이프라인 오케스트레이터 | `.omx/plans/`, 검증 상태 |
| 실행 | `$ultraqa` | QA 순환: test → verify → fix → repeat | 검증된 완료 상태 |
...
| 카테고리 | 명령 | 역할 |
|---|---|---|
| 단축 | `$analyze` | 읽기 전용 저장소 심층 분석 |
| 단축 | `$code-review` | 종합 코드 리뷰 |
| 단축 | `$ai-slop-cleaner` | AI 생성 코드 정리/리팩터링 |
| 단축 | `$visual-ralph` | 시각적 프론트엔드 배달 루프 |
| 단축 | `$git-master` | Git 전문가 (atomic commit 등) |
| 단축 | `$autoresearch` | 상태 유지 연구 루프 |
| 단축 | `$performance-goal` | 성능 최적화 goal 워크플로우 |
| 유틸리티 | `$cancel` | 활성 OMX 모드 취소 |
| 유틸리티 | `$skill` | 스킬 탐색 및 관리 |
...
+------------------------------------------------------------------+
|                            OMX Layer                             |
|  interview  plan  autopilot  team  ralph  pipeline  ultragoal    |
+--------------------------------+---------------------------------+
                        |
                        v
+------------------------------------------------------------------+
...
+------------------------------------------------------------------+
```
OMX는 Codex CLI를 대체하지 않습니다. 인터뷰, 계획, 자율 실행, 병렬 실행, 지속 루프, 파이프라인, 장기 목표 추적을 Codex 위에 얹어 작업 순서와 기억 장치를 제공합니다.
현재 OMX는 네이티브 Codex 훅 (`.codex/hooks.json`)을 정식 수명주기 인터페이스로 사용합니다. `.omx/hooks/*.mjs`는 OMX 플러그인 훅입니다.
...
Structured Spec (.omx/specs/)
    ...
    +---> $ralplan / $plan (manual control)
          ...
          Done or Retry
```
`$autopilot`은 ralplan → ralph → code-review를 자동 순환하며, code-review가 불통과하면 ralplan으로 돌아가 재계획합니다.
...
`$ralplan`은 `$plan`의 별칭(alias)입니다. 기능은 동일하며, `$plan`이 정식 명칭입니다.
...
`$plan`은 `$ralplan`의 정식 명칭이며 같은 계획 정책을 다룹니다.
- 계획을 얼마나 잘게 나눌지 정합니다.
- 사용자 승인 없이 진행 가능한 범위를 제한합니다.
- 위험한 명령이나 큰 리팩터링 앞에 게이트를 둡니다.
- `$team`이나 `$ralph`가 따라야 할 실행 기준을 제공합니다.
즉 `$ralplan`과 `$plan`은 같은 계획 명령을 가리키며, 새 문서에서는 `$plan`을 정식 명칭으로 볼 수 있습니다.
...
ralplan → ralph → code-review를 자동 순환하는 완전 자율 배달 루프

---

## `$autopilot`의 목적

`$autopilot`은 사용자 개입 없이 계획부터 리뷰까지 전체 파이프라인을 자동 실행합니다.
- `$ralplan`으로 계획을 수립합니다.
- `$ralph`로 구현과 검증을 진행합니다.
- `$code-review`로 품질을 평가합니다.
- 리뷰가 불통과이면 `$ralplan`으로 돌아가 문제점을 반영해 재계획합니다.
- 리뷰가 통과할 때까지 이 루프를 반복합니다.
핵심 계약: `$ralplan → $ralph → $code-review` (불통과 시 `$ralplan`으로 회귀)

---

## `$autopilot`를 쓸 때와 쓰지 않을 때

| 상황 | 명령 | 이유 |
|---|---|---|
| 구체적인 아이디어가 있고 손을 떼고 싶다 | `$autopilot` | 전체 파이프라인 자동화 |
| 계획 단계까지만 제어하고 싶다 | `$ralplan` → `$ralph` | 실행 전 승인 경유 |
| 병렬 실행이 필요하다 | `$team` | worker 기반 병렬 처리 |
| 한 가지에 집중해서 끝내고 싶다 | `$ralph` | 단일 소유자 지속 루프 |
| 여러 목표를 장기 추적한다 | `$ultragoal` | 다중 goal 관리 |
...
```bash
.omx/
  state/              # 세션 범위 상태 (state/sessions/{sessionId}/...)
  context/            # 결정, 제약, 문맥
  specs/              # $deep-interview 결과
  plans/              # $ralplan/$plan 결과
  logs/               # 명령별 실행 로그
  wiki/               # 검색 우선 지식 베이스
  ultragoal/          # $ultragoal 다중 목표 상태
  goals/              # NEW: autoresearch/, performance/ 하위
  hooks/              # NEW: OMX 플러그인 훅 (.mjs)
  notepad.md          # NEW: 빠른 메모
  project-memory.json # NEW: 프로젝트 수준 기억
```
...
`.omx/state/`는 세션 범위의 현재 실행 상태를 담습니다.
```bash
.omx/state/
  sessions/{sessionId}/
    ralph-state.json
...
- 어느 계획 단계에 있는지 추적합니다.
- worker pane의 상태와 검증 결과를 요약합니다.
- 명령별 재개 지점을 세션 단위로 남깁니다.
상태는 이제 세션 범위로 관리됩니다. `.omx/state/sessions/{sessionId}/` 아래에 각 명령의 상태 파일이 생성됩니다.
...
| 자율 배달 | 사용자가 중간에 개입 | `$autopilot`이 계획→실행→리뷰 자동 순환 |
...
# 2. OMX 초기 설정 (스킬, 프롬프트, 훅, AGENTS.md)
omx setup
...
omx --madmax --high
```
버전 업그레이드는 `omx update`로 진행합니다. 자동화나 CI처럼 대화형 세션이 아닌 환경에서는 `omx exec`로 비대화형 실행을 구성할 수 있습니다.
...
- `$autopilot`은 ralplan → ralph → code-review를 자동 순환하며 완전 자율 배달을 제공합니다.
- `$team`은 tmux leader/worker 모델로 실제 병렬 실행을 구성합니다.
- `$ralph`는 architect 검증이 통과할 때까지 지속 실행합니다.
- `$pipeline`은 구성 가능한 파이프라인으로 다단계 워크플로우를 오케스트레이션합니다.
